...
if __name__ == "__main__":
    ...
    while True:
        ...
        trades = mongodb_service.query(TRADE_COLLECTION, ticker_filter)

        avg_position = None
        if len(trades) > 0:
            avg_position = calculate_cost_basis(ticker_pair, trades)
        ...
        if avg_position != None: 
            profit = Decimal(((ask * avg_position.shares) - (avg_position.price * avg_position.shares) - (avg_position.fee*2)) / (avg_position.price * avg_position.shares))
        ...
        if take_profit:
            ...
            sell_order = create_sell_order(exchange, ticker_pair, float(avg_position.shares), float(ask))
            if (sell_order != None):
                ...
                mongodb_service.insert_one(SELL_ORDERS_COLLECTION, closed_position)
                mongodb_service.delete_many(TRADE_COLLECTION, ticker_filter)
